	...
	try {
		ytdl(handleReply.link[event.body - 1])
			...
			.on("close", () => {
				...
				else return api.sendMessage({attachment: createReadStream(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`)}, event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`))
			})
		...
		try {
			...
			ytdl(args[0])
				...
				.on("close", () => {
					...
					else return api.sendMessage({attachment: createReadStream(__dirname + `/cache/${id}.m4a`)}, event.threadID, () => unlinkSync(__dirname + `/cache/${id}.m4a`))
				})
				.on("error", (error) => api.sendMessage(getText("returnError", error), event.threadID, event.messageID));
		}
